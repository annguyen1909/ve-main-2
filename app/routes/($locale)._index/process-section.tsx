...
export function WorkProcess({ items = defaultItems, className, ...props }: WorkProcessProps) {
  ...
  return (
    <section className="pt-12 sm:pt-16 md:pt-20 relative px-4 sm:px-6 md:px-8 mb-12 md:mb-0">

      {/* Background grid decoration - Fixed position container - hidden on mobile */}
      <div
        className="hidden md:block absolute max-w-7xl mx-auto left-0 right-0 pointer-events-none"
        style={{
      ...
      <div className="relative">
        ...
        <div className={cn("w-full max-w-7xl mx-auto", className)} {...props}>
          <div className="relative w-full">
            ...
            <div className="relative grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-[repeat(22,1fr)] gap-6 sm:gap-8 md:gap-0">
              {items.map((item, index) => {
                const isVisible = visibleItems[index];
                
                // Apply column spans only for lg+ screens via Tailwind arbitrary values
                const colSpanClasses = [
                  'lg:[grid-column:span_5]',
                  'lg:[grid-column:span_7]', 
                  'lg:[grid-column:span_5]',
                  'lg:[grid-column:span_5]'
                ];
                      ...
                      <div className={cn(index < 2 && "md:ml-2")}>
                        {item.subtitles.map((text, idx) => (
                          ...
                          >
                            {/* Vertical line above */}
                            {idx > 0 && (
                              <div className="w-px bg-gray-500 h-12 sm:h-16 md:h-20 mt-1" style={{ width: '1px' }}></div>
                            )}
                            {idx === 0 && (
                              <div className="w-px bg-gray-500 h-3 sm:h-4 mt-1" style={{ width: '1px' }}></div>
                            )}
